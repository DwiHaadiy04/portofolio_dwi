...
const Navbar = ({ hidden = false }) => {
  ...
  return (
    <nav className="navbar relative z-50 py-7 flex items-center justify-between px-6 md:px-12">
      ...
      {/* Menu */}
      <ul
        className={`flex items-center sm:gap-10 gap-4 flex-col bg-white/10 backdrop-blur-md rounded-2xl shadow-2xl p-6 w-72 sm:w-80 max-w-[90vw] mx-4
          md:static md:flex-row fixed bottom-4 left-1/2 -translate-x-1/2 md:translate-x-0 md:translate-y-0 md:bottom-auto md:top-auto
          md:opacity-100 md:bg-transparent md:backdrop-blur-none md:shadow-none md:p-0 md:w-auto md:mx-0 md:rounded-none
          transition-all duration-300 md:transition-none
          ${active ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0 md:opacity-100 md:translate-y-0"}`}
      >
